...
import { Note, addInterval, TuningPresets, Tuning, fretboardNote, Interval, noteInterval, getIntervalPositions } from './index';
...
test('getIntervalPositions', (t) => {
  ...
  const perfect5ths = getIntervalPositions(tuning, {
    position: [6, 3],
  ...
  actual.forEach(actualPos => {
    t.ok(perfect5ths.find(pos => pos[0] === actualPos[0] && pos[1] === actualPos[1]), `Perfect 5th of G found at ${actualPos}`);
  })
